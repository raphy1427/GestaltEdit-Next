...
NSDictionary<NSString *, id> *BadQueryBridgeDiagnostics(void)
{
    BadQueryAPI *api = BadQuerySharedAPI();
    return @{
        @"libraryLoaded": @(api->library != NULL),
        @"container_query_create": @(api->create != NULL),
        @"container_query_set_class": @(api->setClass != NULL),
        @"container_query_set_group_identifiers": @(api->setGroupIdentifiers != NULL),
        @"container_query_operation_set_flags": @(api->setFlags != NULL),
        @"container_query_operation_set_part": @(api->setPart != NULL),
        @"container_query_operation_set_part_domain": @(api->setPartDomain != NULL),
        @"container_query_get_single_result": @(api->getSingleResult != NULL),
        @"container_query_free": @(api->freeQuery != NULL),
        @"container_copy_sandbox_token": @(api->copySandboxToken != NULL),
        @"sandbox_extension_consume": @(api->consumeSandboxExtension != NULL),
        @"sandbox_extension_release": @(api->releaseSandboxExtension != NULL),
        @"allRequiredSymbols": @(BadQueryBridgeAvailable())
    };
}
...
{
    return [self leaseForPath:path stage:NULL error:error];
}

+ (instancetype)leaseForPath:(NSString *)path
                       ...
                       error:(NSString **)error
{
    ...
    if (!BadQueryBridgeAvailable()) {
        if (stage) *stage = @"symbol-discovery";
        if (error) *error = @"bad_query ContainerManager API unavailable";
        return nil;
    }

    if (stage) *stage = @"query-create";
    BadQueryAPI *api = BadQuerySharedAPI();
    ...
    if (stage) *stage = @"query-configure";
    ...
    if (stage) *stage = @"sandbox-consume";
