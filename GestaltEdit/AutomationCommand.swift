...
enum AutomationCommand {
    ...
    static func runIfNeeded() {
        ...
        guard regulatoryModel.range(of: #"^A[0-9]{4}$"#, options: .regularExpression) != nil else {
            finish(marker: failureMarker, message: "Invalid regulatory model: \(regulatoryModel)")
        }

        // GestaltEdit Next must never allow the command-line automation path
        // to bypass Research Mode's write lock on an unverified iOS build.
        guard GestaltAccess.isRunningSupportedOS() else {
            finish(
                marker: failureMarker,
                message: "Write automation is disabled on this iOS build (\(GestaltAccess.currentOSBuild()))."
            )
        }
